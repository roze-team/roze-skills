---
name: roze-skills
description: Roze Rust microservice framework knowledge for AI agents. Use when working with roze-team/roze, rozectl, Roze .api, .proto, .ent, model/search schemas, generated Rust REST/RPC/stream/AI services, Roze native HTTP/roze-http, native WebSocket routes, Roze crates such as roze-rpc/roze-service/roze-config/roze-gateway/roze-ai/roze-rate-limit/roze-redis/roze-mq/roze-kafka/roze-search/roze-query/roze-migration/roze-orm/roze-report/roze-storage/roze-transaction-sql, managed upstream RPC clients and roze-service.yaml dependency manifests, Rust ownership/lifetime/error/async/concurrency/performance/unsafe issues inside Roze code, semantic Roze errors, string business response codes, typed application configuration, service lifecycle/bootstrap, health/readiness probes, gRPC health/routing, service governance, gateway native HTTP policy, permission/idempotency middleware, trusted client IP, request-body limits, Redis standalone/Cluster topology, Redis idempotency, high-rate distributed rate limiting, strict ServiceConfig validation, persistent SQL outbox, database sharding with RozeShard, scoped SeaORM model transactions, cache consistency, S3-compatible and Qiniu Kodo object storage, reporting/charts and ReportDataSource hooks, AI runtime/generation, model/search generation, ent-style query/create/update/update_where/delete/projection/aggregate/hook/policy/mixin builders, OpenAPI/Web SDK generation, contract diff, gate checks, generator extensions, API plugins, generated ops assets, generated production reference systems, competitive benchmarks, release gates, production release audits, production evidence verification/promotion, smoke/soak tests, or troubleshooting Roze project conventions.
---
...
- Prefer Roze built-in generators, crates, middleware, config, governance, context, error, health, metrics, tracing, registry, gateway, AI runtime, rate limiting, cache, MQ, DTM, RPC-client, and search/model helpers before introducing custom infrastructure.
...
- [references/service-patterns.md](references/service-patterns.md): use for generated REST/RPC/stream layouts, Roze native HTTP handlers/router/extractors/responses, server adapters, managed upstream RPC clients, logic placement, service context, typed application config, numeric/string response envelopes, validation, context and client-IP propagation, permissions, idempotency, semantic errors, middleware/body limits, health endpoints, lifecycle/bootstrap, and generated ownership.
- [references/data-search-patterns.md](references/data-search-patterns.md): use for `.ent` model generation, SeaORM-default and Toasty-explicit generation, generated model manifest dependencies, SQL constraints and Mongo inspection, explicit `RozeShard` database sharding, scoped SeaORM model transactions, model migrations, deterministic model fixtures/seeds, ent-style repository/client/query/mutation/conditional-update/projection/aggregate/hook/policy/mixin builders, SeaORM primary/replica read-source selection, decimal/smallint mappings, search DSL/inspection, and Elasticsearch/OpenSearch/Meilisearch support.
...
- [references/governance-operations.md](references/governance-operations.md): use for typed application config, strict ServiceConfig validation, database direct/proxy/sharded modes, registry, gateway, Redis standalone/Cluster topology, distributed rate limiting, MQ, DTM, SeaORM/Toasty transactional persistent outbox, cache consistency, object storage, Qiniu Kodo/S3-compatible storage, read-model composition, lifecycle, metrics, tracing, release gates, production evidence, smoke/soak scripts, and hot-path expectations.
...
- Framework behavior should come from Roze first: prefer built-in middleware, `roze_context`, `roze_result`, `roze_error`, `roze_health`, `roze_config`, `roze_rpc`, `roze_gateway`, `roze_http`, `roze_ai`, `roze_rate_limit`, `roze_mq`, `roze_cache`, `roze_local_cache`, `roze_singleflight`, `roze_query`, `roze_storage`, `roze_transaction`, `roze_search`, `roze_report`, generated RPC clients, service dependency manifests, and generated repositories before adding local equivalents.
...
- Do not replace built-in Roze middleware, context, config, health, registry, rate limiting, cache, MQ, DTM, metrics, tracing, or search/model helpers with local implementations unless the active checkout lacks the needed capability and the gap is documented.
