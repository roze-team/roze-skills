...
- [references/governance-operations.md](references/governance-operations.md): use for typed application config, strict ServiceConfig validation, always-on redaction, stable structured events, database direct/proxy/sharded modes, registry, gateway, Redis standalone/Cluster topology, distributed rate limiting, MQ, external DTM integration, SeaORM/Toasty transactional persistent outbox, cache consistency, provider-neutral SMS, object storage, Qiniu Kodo/S3-compatible storage, read-model composition, VM/systemd deployment layout, lifecycle, metrics, tracing, release gates, production evidence, smoke/soak scripts, and hot-path expectations.
