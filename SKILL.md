...
- [references/service-patterns.md](references/service-patterns.md): use for generated REST/RPC/stream layouts, Roze native HTTP handlers/router/extractors/responses, server adapters, managed upstream RPC clients, logic placement, service context, typed application config, validation, context propagation, permissions, idempotency, semantic errors, middleware, health endpoints, lifecycle/bootstrap, and generated ownership.
- [references/data-search-patterns.md](references/data-search-patterns.md): use for `.ent` model generation, Toasty, SeaORM, generated model manifest dependencies, SQL/Mongo inspection, explicit `RozeShard` database sharding, model migrations, deterministic model fixtures/seeds, ent-style repository/client/query/mutation/conditional-update/projection/aggregate/hook/policy/mixin builders, SeaORM primary/replica read-source selection, decimal/smallint mappings, search DSL/inspection, and Elasticsearch/OpenSearch/Meilisearch support.
...
rozectl model generate model/schema.ent --out services/user-api --format ent --orm sea-orm --switch-orm
