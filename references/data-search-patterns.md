...
- PostgreSQL `TIMESTAMP` and `TIMESTAMPTZ` preserve their distinction as `.ent` `timestamp` and `timestamptz`; SeaORM output uses chrono-backed `DateTime` and `DateTimeUtc`, and generation enables SeaORM `with-chrono` plus chrono `clock` and `serde` features, merging them into existing dependencies when present. Toasty output uses `jiff::civil::DateTime` and `jiff::Timestamp`, including nullable fields, and generation enables Toasty's `jiff` feature plus a direct serde-enabled `jiff` dependency across mutations, predicates, ordering, projections, and grouped counts.
