...
Use the native `roze-dtm` coordinator and `apps/roze-dtm` control plane before building local distributed-transaction orchestration. TCC is the default; Saga is for long-running eventual-consistency workflows. Production requires persistent storage, a secret control token of at least 32 bytes, a deployment-unique worker ID, and a recovery lease at least twice the recovery interval. Memory storage is development-only; SQLite is the current persistent implementation.

Require `Authorization: Bearer <token>` for every `/v1/**` DTM endpoint while leaving health/startup/readiness/metrics probes separate. `allowed_branch_origins` is required in every environment and accepts only exact HTTP(S) origins without paths, wildcards, or credentials. Validate Action/Confirm/Cancel/Compensate URLs both at submission and call time, and reject redirects. Bound transactions to 1-100 unique branches, bounded IDs/metadata/URLs, a 1-second to 24-hour timeout, and the native 2 MiB request-body limit.

Recovery must use persistent leases and advance only safely replayable states. Branch barriers deduplicate `gid + branch_id + operation`; Cancel before Try is an empty rollback, and failed branches use bounded exponential backoff. Do not force-replay ambiguous `Trying` or `Succeeding` states. DTM control-plane mutations and recovery emit stable `roze.audit` events without tokens, request payloads, branch bodies, or raw dependency errors. PostgreSQL/MySQL/Redis stores, external lease backends, XA, two-phase messages, and a dashboard are not current stable capabilities.
