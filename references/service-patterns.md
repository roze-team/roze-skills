...
Generated services and framework crates should not expose Axum types. Roze native HTTP owns the public REST surface: `roze_http::Router`, method routers, `roze_http::body`, typed extractors such as `Path`, `Query`, `Json`, `Form`, `State`, and `Extension`, `IntoResponse`, `Response`, `ErrorResponse`, and Tower-compatible middleware helpers.
