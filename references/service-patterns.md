...
`roze_service::ServiceGroup` provides `RuntimeService`, `FnService`, `ServiceGroupConfig`, `ServiceGroupHandle`, and `LifecycleState` with `Starting`, `Running`, `Draining`, `Stopped`, and `Failed` phases. Use it for HTTP/RPC/consumer/job/background services that must share one shutdown signal and stop-hook timeout.

Do not keep `/readyz` green during shutdown. If adding custom background services, wire them into the shared lifecycle so readiness, draining, stop hooks, and task timeouts remain observable and testable.
